...
- **完整覆盖率**（`pnpm run test:coverage`）：本地和继承的上游检查对 `packages/*/*/src` 强制按文件 100% 覆盖；自动 Community CI 不运行它。未覆盖的行通常表示死代码，而不是缺少测试。覆盖率证明代码被执行过，不证明交付行为正确。`packages/shell/pwsh-local/src` 达到 100% 需要真实的 `pwsh`；否则其执行器套件会自动跳过，`vitest.config.ts` 会豁免该文件。
- **真实 API e2e**（`pnpm run test:e2e`）：真实提供方测试使用各提供方的密钥（`DEEPSEEK_API_KEY`、`EXA_API_KEY`、`PERPLEXITY_API_KEY` 等），本地套件缺少对应密钥时会自动跳过。手动 GitHub 工作流要求 `DEEPSEEK_API_KEY_EXTERNAL`，缺少时会在预检中失败，从而避免全部跳过的绿色结果（[Agent Note](../.agents/notes/implemented/testing/2026-06-19-real-api-e2e-ci.md)）。
...
- **Web 浏览器快照**（`pnpm run test:web`）：Chromium 将回放输出与 `apps/web/tests/snapshots/` 比较。继承的上游定义把它作为设置 `DSH_SNAPSHOT=replay` 的只读 Linux PR（Pull Request）门禁；自动 Community CI 不运行它。record 和 refresh 留在本地，每处 diff 都须评审（[通道](../.agents/notes/implemented/testing/2026-07-24-web-gui-browser-e2e-lane.md)、[上游决策](../.agents/notes/implemented/testing/2026-07-30-web-browser-snapshot-ci-gate.md)）。`test:web` 会[先构建](../.agents/notes/implemented/bug-fix/2026-07-28-themed-scrollbars-and-reserved-gutter.md)以交付插件 CSS。
...
DSH Context Map 是独立于 DeepSeek 的项目。提供方行为变化或发布需要验证真实兼容性时，请运行真实 API 测试：无密钥测试证明底层通路，带密钥测试证明真实模型。覆盖文件写入、多个轮次的对话、工具使用和流中取消。价值最高的是**冒烟测试**：启动真实示例、发送一条提示词并检查外部世界；它们能捕获 mock 无法发现的「单元测试全绿、产品却坏了」问题（[事故复盘](postmortem/0001-acp-default-export-drops-inject.md)）。本地自动跳过让无密钥贡献者不受阻塞；手动 GitHub 工作流会在预检时拒绝缺少密钥的运行。每个示例都提供无密钥和带密钥冒烟测试（[说明](../examples/AGENTS.md)）。
...
- 基于构建产物的通道（包括继承的上游参考）通过共享双模式启动器，从构建后的 `lib/` 运行示例与 Cordis 配置子进程。不要为它们手写 `--import tsx`。
- 不加载 Cordis 的协议与操作系统 fixture 直接通过 Node 运行使用可擦除语法的 `.ts` 文件，不经过 tsx 或根路径映射。
- 只有测试对象本身是源码路径解析时，才可以选择 `src`；在测试中写明这一约定。

## 何时需要快照测试

每项非平凡的模型可见、协议可见或人类可见变更，都必须在同一 PR 中，通过可运行示例所属的快照套件添加或更新无密钥场景。包测试、e2e 断言、mock 与仅测试组合、PR 理由都不能取代组装后的 transcript；必要时应扩展 harness。ACP 场景在 `examples/<name>/tests/snapshots/` 使用 [`dsh-acp-snapshot`](../packages/test-support/acp-snapshot/README.md) 工厂（`examples/acp-agent` 为主套件）；`examples/headless-agent` 拥有规范事件 JSONL 快照与回放 fixture。`pwsh-tool-turn` 启动真实 `pwsh`，缺少时跳过。已完成的交互式终端旅程使用 `apps/cli/tests/snapshots/` 下的 JSONL 场景；瞬态呈现使用包内语义矩阵，输入、Loader 选择或终端清理发生变化时还要添加 PTY 用例。Web GUI 旅程使用 `apps/web/tests/snapshots/`。两个 SDK 分别投影 agent loop、会话生命周期与 `SessionEventMap`，因此改动这些内容时要同时更新两者：`examples/jsonrpc-agent/tests/snapshots/` 属于 TypeScript；`scripts/snapshots/python-sdk-single-exe/` 属于 Python。后者的 `python-runtime` job 保留在继承的上游定义中，不属于自动 Community CI。新的能力 seam、生命周期变体或 transcript 呈现接口在计划阶段就要列出每个覆盖层级，并在实现前证明 harness 能表达它们。
