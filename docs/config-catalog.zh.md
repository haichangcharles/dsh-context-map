...
- `@deepseek-ai/dsh-contextify`——依赖 `agents` · `sessions` · `sessionPersistence` · `contextCompiler` · `llm`（[`packages/context/contextify/src/index.ts`](../packages/context/contextify/src/index.ts)）
