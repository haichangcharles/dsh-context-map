...
No reveal control appears when no usable Session is active. On a viewport where the native column solver cannot grant details width, the existing solver remains authoritative; this feature does not introduce an overlay drawer or change minimum-width rules.
