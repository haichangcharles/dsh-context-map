...
```ts type-equiv
/** Identity of the explicitly selected context compiler recorded in a request header. */
interface RequestContextCompilerDescriptor {
...
```ts type-equiv
/**
 * Logged request state outside derived history: call config, tools, and an
 * optional context compiler. The system prompt is derived history, carried by
 * `system/message`. The latest full `request/header` reconstructs this header;
