...
Before each model call, `agent-loop` uses the explicitly selected [`dsh-context-compiler`](../../packages/context/context-compiler/README.md) provider when present; the default Surface path retains upstream message derivation and header shape. The loop records non-default compiler metadata in `request/header` and recompiles after request recovery changes durable history.
