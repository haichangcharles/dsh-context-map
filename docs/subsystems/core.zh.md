...
每次模型调用前，`agent-loop` 使用显式选择的 [`dsh-context-compiler`](../../packages/context/context-compiler/README.zh.md) 提供方；默认 Surface 路径保留上游消息派生和 Header 结构。Loop 在 `request/header` 中记录非默认 Compiler 元数据，并在请求恢复改变持久历史后重新编译。
