...
The historical upstream [larger-runner topology](../../archived/process/2026-07-22-evidence-based-larger-hosted-runners.md) gave the static and built-consumer inventories separate jobs, but the static job owned their shared build. It uploaded the emitted tree only after every static gate completed, and the consumer job declared a job-level dependency before restoring that tree. Compiled-output snapshots and package-compatibility checks genuinely require a complete build; they do not require runtime-closure checks, documentation generation, module-graph verification, or Knip.
