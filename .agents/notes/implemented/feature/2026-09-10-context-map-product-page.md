...
The product homepage is static HTML, CSS and browser JavaScript under `product-site/public`. Its dark developer-tool layout uses near-black neutral surfaces, readable text, quiet borders and restrained green accents, informed by Linear and Zed; copy is original. English is the default, with a URL-backed Chinese switch. A screenshot gallery replaces the single-purpose video. A prepared launch scenario shares one starting point across two native branches. The overview shows the session family, followed by five visible sections: Branch from Here, cross-branch context, Fast/Deep Review and undo, Archive/Restore, and the Prompt Dashboard including optional Automatic Branch review. Each section explains when to use it and names the actual control. Images link directly to their full-resolution originals. Screenshots use the running application; conversations and the recommendation are explicitly labelled as prepared samples rather than live AI results.
