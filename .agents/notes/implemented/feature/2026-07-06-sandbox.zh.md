...
Landlock launcher 源码和包家族位于 `native/landlock-run`，与 harness 消费方同仓，并属于根 pnpm workspace。已归档的[仓库内集成决策](../../archived/process/2026-08-06-in-repository-landlock-release.md)记录共享锁文件、原生构建和打包演练的设计依据；[社区源码发布决策](../process/2026-09-07-community-source-release-boundary.md)负责规定受支持 Workflow 不发布 Package。平台二进制由 npm 兼容 Manifest 选择，入口包拥有路径解析、探测、CLI（命令行界面）参数、致命前缀和部分强制执行通知，而 harness 将沙箱模式映射为授权。将入口点与其二进制一起版本化，使探测解析和启动语法保持对齐。
...
- **launcher 在源码中是 workspace 依赖，在上游包分发中会成为 NPM 依赖。** 本社区仓库将经审查的 C 源码、原生 CI 构建和字节固定的本地 tarball 一起作为兼容性演练测试，但不会发布它们；真实内核 e2e 测试环节会验证这些安装字节的实际行为。
