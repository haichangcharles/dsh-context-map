...
The Landlock launcher source and package family live at `native/landlock-run`, next to the harness consumers and inside the root pnpm workspace. The archived [in-repository integration decision](../../archived/process/2026-08-06-in-repository-landlock-release.md) records the shared-lockfile, native-build, and pack-rehearsal rationale; the [community source-release decision](../process/2026-09-07-community-source-release-boundary.md) owns the absence of package publication from supported workflows. Platform binaries are selected by npm-compatible manifests, and the entry package owns path resolution, probing, CLI flags, the fatal prefix, and the partial-enforcement notice while the harness maps sandbox modes to grants. Versioning the entry point with its binaries keeps probe parsing and launch syntax aligned.
...
- **The launcher is a workspace dependency in source and would be an npm dependency in an upstream package distribution.** This community repository tests reviewed C source, native CI builds, and byte-pinned local tarballs together as a compatibility rehearsal but does not publish them; the real-kernel e2e legs vouch for behavior through those installed bytes.
