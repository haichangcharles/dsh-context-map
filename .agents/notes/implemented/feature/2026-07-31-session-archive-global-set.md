...
- Restore extension: `unarchiveSession(id)` is the serialized, idempotent inverse mutation and every RPC/frame continues to carry the complete set. It removes only the stable Session id and never restores a Workspace snapshot, so current Workspace grouping/order wins after changes made during archival. The sidebar exposes a searchable, current-Workspace-grouped restore subview whose rows never open Sessions; unary success remains pending until the authoritative set projection removes the id. Context Map hides archived-only nodes but retains their dragged coordinates against the complete native family, clearing only hidden canvas selection.
...
Archived sessions now have a sidebar restore surface but still no destructive deletion capability. Data and accounting slots remain intact throughout archive/restore. Tests pin idempotence, ordered archive/restore races, disappearance while archived, stale list responses after a restore echo, failed retry retention, assembled UI selection stability, and Context Map coordinate restoration.
