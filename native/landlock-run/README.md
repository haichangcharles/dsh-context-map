...
A [Landlock](https://landlock.io/) self-restrict-then-exec launcher for confining subprocesses on Linux. Upstream DeepSeek Harness distributes it as prebuilt per-platform npm packages plus a thin JS entry package; DSH Context Map retains that package contract for source-build compatibility but does not publish the inherited `@deepseek-ai/*` packages.
...
The upstream package family uses an entry package plus platform optional packages:
...
Binaries are git-ignored and built natively per architecture — locally for your own machine and by compatibility CI on its selected runners. The inherited package rehearsal is documented in [docs/release.md](docs/release.md); this repository's supported release is source-only.
