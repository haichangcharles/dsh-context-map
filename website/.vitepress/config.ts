...
const sharedTheme: Pick<DefaultTheme.Config, 'search' | 'socialLinks' | 'editLink'> = {
  ...
  socialLinks: [
    { icon: 'github', link: 'https://github.com/haichangcharles/dsh-context-map' },
  ],
  editLink: {
    pattern: ({ frontmatter }: PageData) => {
      ...
      return `https://github.com/haichangcharles/dsh-context-map/edit/master/${editSource}`
    },
    text: '在 GitHub 上编辑此页',
  },
}
...
.dsh-product-name { color: var(--vp-c-text-1); font-size: 16px; font-weight: 600; white-space: nowrap; }
.dsh-tag {
  display: inline-flex;
  align-items: center;
...
/**
 * Navigation-bar title: the DSH Context Map name and release-stage tag.
...
function siteTitle(previewTag: string): string {
  return `<span class="dsh-lockup"><span class="dsh-product-name">DSH Context Map</span><span class="dsh-tag">${previewTag}</span></span>`
}

export default withMermaid({
  title: 'DSH Context Map',
  description: '基于 DeepSeek Harness 的可视化对话上下文工作区',
  base,
  head: [
  ...
  vite: {
    ...
    plugins: [
      {
        name: 'dsh-context-map-doc-projector',
