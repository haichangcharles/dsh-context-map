...
> **Inherited upstream component.** DSH Context Map keeps this SDK source for DeepSeek Harness compatibility but does not publish Python packages. This page documents the inherited source snapshot, not the current upstream PyPI contract. Run DSH Context Map itself from the repository source using the [project README](../../README.md).
...
This source snapshot intentionally provides no unpinned PyPI command. For a Python deployment, use the [current upstream SDK documentation](https://github.com/deepseek-ai/deepseek-harness/tree/master/python/sdk) and keep the source revision, SDK distribution, and runtime carrier versions specified there together. Do not combine a current upstream distribution with this checkout's examples. Upstream artifacts do not include Context Map features; the import module described by this snapshot is `deepseek_harness`.
...
The [inherited Python SDK reference](../../docs/user/guide/python-sdk.md) documents same-checkout source validation without the Web UI; it is not a supported DSH Context Map installation path. The [`jsonrpc-agent` example](../../examples/jsonrpc-agent/README.md) owns the complete standalone Cordis file used there.
...
The same behavior can be selected for the runtime subprocess with `DSH_CORDIS_CONFIG`. The injection lives in `HarnessClient.start()`, so the low-level client's default launch gets it too: when the launch resolves to the bundled runtime and neither `cordis` nor a non-empty `DSH_CORDIS_CONFIG` is set (the runtime treats an empty value as absent, and so does the injection check), the bundled default configuration is used; an explicit `runtime_bin`, `bridge_bin`, or `launch_args_override` disables the injection entirely. See the [sdk-runtime README](../sdk-runtime/README.md) for this snapshot's runtime carriers (production exe vs dev-only node closure).
