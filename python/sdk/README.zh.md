...
> **上游继承组件。** DSH Context Map 为兼容 DeepSeek Harness 而保留此 SDK 源码，但不发布 Python 包。本页记录的是继承源码快照，并非当前上游 PyPI 契约。DSH Context Map 本身请按照[项目 README](../../README.md) 从仓库源码运行。
...
此源码快照刻意不提供未固定版本的 PyPI 命令。Python 部署请使用[当前上游 SDK 文档](https://github.com/deepseek-ai/deepseek-harness/tree/master/python/sdk)，并保持其中指定的源码 revision、SDK 分发包与运行时载体版本一致。不要把当前上游分发包与本次检出的示例混用。上游产物不包含 Context Map 功能；此快照描述的导入模块是 `deepseek_harness`。
...
[上游继承 Python SDK 参考](../../docs/user/guide/python-sdk.md)介绍无需使用 Web UI 的同检出源码验证；它不是 DSH Context Map 支持的安装路径。该参考所用的完整独立 Cordis 配置文件位于 [`jsonrpc-agent` 示例](../../examples/jsonrpc-agent/README.md)中。
...
也可以通过 `DSH_CORDIS_CONFIG` 为运行时子进程指定配置。注入逻辑位于 `HarnessClient.start()`，因此底层客户端按默认方式启动时也具有该行为：如果启动方式最终解析为内置运行时，且既没有设置 `cordis`，也没有设置非空的 `DSH_CORDIS_CONFIG`（运行时将空值视为未设置，注入检查也是如此），系统就会使用内置默认配置；显式指定 `runtime_bin`、`bridge_bin` 或 `launch_args_override` 时，则会完全禁用该注入。此快照的运行时载体（生产用 exe 与仅限开发的 `node` 闭包）见 [sdk-runtime README](../sdk-runtime/README.md)。
