...
运行时兼容性产物仅提供 wheel 包。构建会连同纯 SDK wheel 包一起生成三个平台 wheel 包：Linux x64、Linux arm64 和 macOS 14 或更高版本的 arm64。只有与仓库版本匹配时，才接受可选的 `python-v<repository-version>` 输入；`0.0.1-rc.1` 之类的仓库预发布版本在 wheel 包文件名和元数据中使用规范化的 PEP 440 写法，例如 `0.0.1rc1`。

## 验证兼容性候选产物

为拉取请求添加 `python-release-dry-run` 标签，或手动运行 GitHub 的 `Package compatibility (upstream Python)` Workflow，即可构建全部四个 wheel 包，在 Python 3.10 和 3.14 上安装 Linux 集合，检查精确文件名和元数据，执行 PyPI 默认单文件大小限制，并保留一份带 SHA-256 哈希的汇总产物。两条路径都没有 Registry 凭据或发布 Job。

DSH Context Map 通过 GitHub Releases 发布带 Tag 的源码，而不是 Python Package。Wheel 输出只是短期兼容性证据，不是受支持的安装渠道；增加项目自有的 PyPI 分发需要单独形成发布决策。
