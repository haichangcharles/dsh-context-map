...
> **Inherited upstream component.** DSH Context Map keeps this runtime-carrier source for DeepSeek Harness compatibility but does not publish wheels. References to wheel distributions, release tags, and production carriers below describe this inherited source snapshot's historical contract, not the current upstream artifact shape. Those artifacts do not include the Context Map UI or extensions.
