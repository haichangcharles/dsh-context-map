...
`session.fork` 将可选的 `atSeq` 事件锚点映射到该锚点处或其后的首个 `turn/end`，使消息操作可包含该消息所在的完整轮次。锚点省略或超过末尾时，选择最后一个已完成轮次；若锚点已在日志中，而其所在轮次仍开放，则返回 `fork-unavailable`，不会向较早位置裁剪。与它互斥的 `beforeSeq` 必须指向一个 `turn/start`，并在该轮之前立即裁剪，使“移到 Branch”流程可以在 child 中重新放入这组 Q&A，而不会先继承它。发布后的子会话会先继承选定的源会话前缀、cwd、日志中最新的 `ModelSelection` 及谱系，再加入源 Workspace。如果附加到 Workspace 失败，`workspace-attach-failed` 会携带已发布的子会话 id，供客户端对账。[SessionStore fork 决策](../../../.agents/notes/implemented/feature/2026-06-30-session-store-fork-api.md)记录了普通锚点为何要映射到该 `turn/end`。
