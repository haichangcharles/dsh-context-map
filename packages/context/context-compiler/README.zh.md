---
description: "选择模型请求使用的持久会话消息，并验证上下文编译器选择。"
...
- [注册 provider](#registering-a-provider)
...
模型看到 active compiler 选出的有序消息，包括持久化的 system 消息，并使用 Harness 常规的工具 schema。非默认 Compiler 元数据记录在 `request/header` 中；默认 Surface 请求保留官方 Header 结构。请求恢复改变历史后会重新编译。
