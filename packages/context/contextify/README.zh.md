...
`contextify` Remote namespace 提供 `get`、分页 `familyPage`、仅建议的 `recommend`、`cancelRecommendation`、`setNodeMode`、批量 `setNodeModes`、`archiveNode`、`prepareBranchSuggestion`、`acceptBranchSuggestion`、`restoreNode`、`reset`、`undo` 和 `redo`。
...
自动 Branch review 对新 Profile 默认开启，也可以在 Prompt Dashboard 关闭。禁用工具的 classifier 会从第一条 human input 到达时与主回答并发启动，仅在成功 Turn 完成后绑定准确的最终 Q&A，因此主回答不会等待它。它的有界 packet 只包含新 input、当前与祖先 objective、最多三个本地最近 Turn、branch 深度／负载以及最多四个 sibling intent；不会包含完整 tree、reasoning 或 tool event。动态高置信度阈值会压制噪声；格式错误、调用失败、关闭、低置信度或已过期的 review 都保持静默。即使关闭自动 review，模型仍可使用 `request_context_branch` 工具：direct human 显式要求开分支时，它会安排同一张 Turn 后确认卡，并且不会声称分支已经创建。接受可见建议时，系统先通过 Harness Host 的原生 fork 路径，在前一个 completed boundary 创建完整且由 Agent 支撑的 child，再把准确 Q&A 回放到 child，并用确定性 placeholder 归档源 input 与最终 output。裸的 live Session 会被拒绝，从而避免普通导航与 persistence resume 竞态。搬迁具有确定性 key，可安全重试；若任一 conversation 已推进，或原生 child 与建议边界不匹配，则拒绝执行。
