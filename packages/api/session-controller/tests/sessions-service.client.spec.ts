...
describe('fork', () => {
  ...
  it('passes an exclusive before-Turn fork boundary to the Host', async () => {
    const b = bench()
    await feedList(b, [{ id: 'source', cwd: '/work' }])
    b.api.onFork = () => Promise.resolve(ok({ sessionId: sid('child') }))

    await expect(b.svc.fork({ sessionId: sid('source'), beforeSeq: 42.8 })).resolves.toBe('child')

    expect(b.api.callsOf('session.fork')).toEqual([{ sessionId: 'source', beforeSeq: 42 }])
  })
