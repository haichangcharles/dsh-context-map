...
describe('sessions.fork', () => {
  ...
  it('rejects ambiguous or non-turn beforeSeq anchors without creating a child', async () => {
    const ctx = await composed()
    try {
      const source = liveAgent(ctx, 'invalid-before-turn', 2)
      const count = ctx.sessions.list().length
      for (const options of [{ beforeSeq: 1 }, { beforeSeq: -1 }, { beforeSeq: 0.5 }, { beforeSeq: 3, atSeq: 1 }]) {
        const response = await remote(ctx).fork(request({ sessionId: source.id, ...options }))
  ...
  it('cuts immediately before a requested turn for move-to-Branch flows', async () => {
    ...
    const secondTurnStart = source.snapshotEvents().find(event =>
    ...
    expect(child?.inheritedEventCount).toBe(secondTurnStart)
