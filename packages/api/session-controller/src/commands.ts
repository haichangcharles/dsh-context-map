...
export class SessionCommandController {
  ...
  async fork(request: SessionForkRequest): Promise<SessionForkValue> {
    ...
    if (request.beforeSeq !== undefined && request.atSeq !== undefined) {
      throw new RemoteError('gateway/bad-request', 'atSeq and beforeSeq are mutually exclusive', {})
    }
    let cut: ReturnType<typeof SessionLogOffset>
    if (request.beforeSeq !== undefined) {
      const beforeSeq = request.beforeSeq
      if (!Number.isSafeInteger(beforeSeq) || beforeSeq < source.inheritedEventCount
        || source.events[beforeSeq]?.type !== 'turn/start') {
        throw new RemoteError('gateway/bad-request', 'beforeSeq must identify an owned turn/start', {})
      }
      cut = SessionLogOffset(beforeSeq)
    } else {
