...
export class ReactLoopAgent implements Agent {
  ...
  constructor(
    ...
    if (loopCtx.get('contextCompiler') === undefined
      && session.snapshotEvents().some(event => event.type === 'context/compiler')) {
      throw new Error('The selected Context Compiler plugin is unavailable')
    }
  ...
  private buildRequest(
    ...
    const compiler = this.loopCtx.get('contextCompiler')
    if (compiler === undefined && session.requestHeader()?.contextCompiler !== undefined) {
      throw new Error('The selected Context Compiler plugin is unavailable')
    }
    const descriptor = compiler?.descriptor(session)
    const compilation = descriptor?.id === 'surface' && descriptor.version === 1 ? undefined : compiler?.compile({
      session,
    ...
    const header = canonicalHeader({
      config,
      ...compilation === undefined ? {} : { contextCompiler: { id: compilation.id, version: compilation.version } },
    ...
    deepFreeze(header)
    const boundaryMessages = compilation === undefined ? session.deriveMessages() : [...compilation.messages]
