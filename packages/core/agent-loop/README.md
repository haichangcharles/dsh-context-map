...
At each entered step, the loop calls `ctx.contextCompiler.compile()` once before entering request recovery. The resulting frozen messages and compiler descriptor are reused by every retry of that step, and the descriptor is recorded in the full `request/header` snapshot. The Agent Loop invariant independently recompiles at dispatch time and rejects either message divergence or a header that names a different compiler. The shipped `surface` provider preserves the previous `session.deriveMessages()` behavior; alternate providers change only model-visible history and never rewrite the durable Session log.
