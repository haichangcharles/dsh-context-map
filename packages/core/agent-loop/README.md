...
At each entered step, the loop calls `ctx.contextCompiler.compile()` before entering request recovery. Ordinary provider retries reuse the resulting frozen messages and compiler descriptor. If recovery or compaction commits a surface replacement and increments `session.surface.replaceGeneration`, the loop recompiles before the next retry so the request reflects the replacement. The effective descriptor is recorded in the full `request/header` snapshot. The Agent Loop invariant independently recompiles at dispatch time and rejects either message divergence or a header that names a different compiler. The shipped `surface` provider preserves the previous `session.deriveMessages()` behavior; alternate providers change only model-visible history and never rewrite the durable Session log.
