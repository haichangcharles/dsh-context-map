...
每个进入的步骤都会在请求恢复逻辑开始前调用 `ctx.contextCompiler.compile()`。普通的 Provider 重试会复用由此得到的冻结消息和 compiler descriptor；如果恢复或压缩提交了 surface replacement，并递增 `session.surface.replaceGeneration`，Agent Loop 会在下一次重试前重新编译，使请求反映这次替换。最终生效的 descriptor 会记录在完整的 `request/header` snapshot 中。Agent Loop invariant 会在分派时独立重新编译，并拒绝消息不一致或 header 指向其他 compiler 的请求。随附的 `surface` provider 保持原先 `session.deriveMessages()` 的行为；替代 provider 只改变模型可见历史，绝不会重写持久 Session 日志。
