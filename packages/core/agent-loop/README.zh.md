...
每个进入的步骤都会在请求恢复逻辑开始前调用一次 `ctx.contextCompiler.compile()`。该步骤的每次重试都会复用由此得到的冻结消息和 compiler descriptor，descriptor 则记录在完整的 `request/header` snapshot 中。Agent Loop invariant 会在分派时独立重新编译，并拒绝消息不一致或 header 指向其他 compiler 的请求。随附的 `surface` provider 保持原先 `session.deriveMessages()` 的行为；替代 provider 只改变模型可见历史，绝不会重写持久 Session 日志。
