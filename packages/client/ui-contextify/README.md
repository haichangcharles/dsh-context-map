...
`Recommend` opens a review sheet over the bottom of the still-visible graph. The isolated Harness Agent can mark proposed Include/Exclude changes and cleanup candidates, but card checkboxes continue to show current effective context until the user accepts. Selection suggestions can be applied individually or as one selected batch. Cleanup has no bulk execution: each candidate must be reviewed, shows evidence and graph impact, and presents before/after text before `Replace with placeholder` is enabled. The resulting card keeps the same node ID and graph structure, displays `Original retained`, and adds `Show original` and `Restore original` to its right-click menu; Show original opens the complete text rather than the card's bounded preview. Closing or dismissing recommendations changes no durable state. A plan change, active Session change, or append in any parent, sibling, or descendant marks the proposal stale, and the Host repeats that check immediately before an accepted mutation.
