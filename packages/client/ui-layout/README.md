...
Shell plugin: three-column AppFrame (drag handles and concession chain) plus the `ctx.layout` panel-geometry service; it registers into the runtime-owned `root` slot and declares `sidebar`, `conversation`, `details`, and `conversation.empty`. The sidebar resize boundary is an invisible hit strip, while the details boundary retains its floating pill; details shrinks toward its floor during concession, after which center absorbs the remaining deficit so an explicitly open details column stays reachable. A closed sidebar retains a 56px control rail while details closes to zero width. With an active non-blank Session, closed details exposes a floating left-arrow on the frame edge; opening the column replaces that reveal control with the native resize handle. The package also seats the theme presenter: it consumes resolved `ctx.theme` snapshots and projects them onto the document (`html { color-scheme }` for native UA chrome, `body[data-ds-dark-theme]` from the active color scheme, the theme's alias tokens as inline variables on body, and one owned `<meta name="theme-color">` whose content follows the computed body background). Measuring after palette and token application keeps the rendered background as the single color authority; disposing the presenter removes its metadata node with its other global writes.
...
- **Concession can squeeze center below its preferred floor** — an explicitly open details column remains reachable at its floor, while exceptionally tiny frames may concede details below that floor; consumers must not read stored panel widths as rendered truth.
