...
DSH Context Map 组合会增加带数量徽标的归档入口，在侧边栏内打开恢复视图，并按当前 Workspace 注册表分组，同时支持按标题、ID 与 Workspace 进行本地筛选。恢复不会打开或切换 Session；它会保持等待状态，直到权威归档集合投影移除该 ID，并在失败时保留可重试的行内错误。原 Workspace 已删除的 Session 会恢复到 Ungrouped；后续 Workspace 重命名、排序和账号变化仍以当前权威状态为准。
