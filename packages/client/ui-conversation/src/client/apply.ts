  ...
  const registerConversationSession = () => slots.register({
    ...
    inject: (sessionId: SessionId, actions: BoundActions<typeof conversationStore>): ConversationSessionInjected => ({
      ...
      bindDraftMirror: (write) => {
        const unmirror = inputHub.shell(sessionId).bindMirror(write)
        const navigation = concreteConversation(ctx).messageNavigation
        const consume = () => {
          const request = navigation.getSnapshot()
          if (request?.sessionId !== sessionId) return
          navigation.set(null)
          activateView(sessionId, 'chat')
          actions.openView('chat', `message:${request.seq}`)
        }
        const unsubscribe = navigation.subscribe(consume)
        consume()
        return () => { unsubscribe(); unmirror() }
      },
      openView: (view, focus) => {
