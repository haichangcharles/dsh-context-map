...
export class ConversationController extends Service implements IConversation {
  /** Pending navigation survives a Session body mounting after selection. */
  readonly messageNavigation = createSnapshotStore<{ sessionId: SessionId; seq: number } | null>(null)

  /** Open Chat at an exact durable message, including unloaded history. */
  revealMessage(sessionId: SessionId, seq: number): void {
    this.ctx.sessions.open(sessionId)
    this.messageNavigation.set({ sessionId, seq })
  }
