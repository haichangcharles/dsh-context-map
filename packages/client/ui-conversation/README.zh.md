...
Scope-addressed conversation service 会向 graph 与 navigation plugin 暴露 message-reveal seam。请求携带原生 Session ID 与持久消息序号；conversation 会打开该 Session、把 view ring 切回 Chat、持续分页加载较早历史直至准确的 user 或 finalized assistant 锚点挂载、把该行滚动到中央，并添加临时高亮。请求 registry 只存在于浏览器内存中，可以跨 Session／view mount handoff 保留，并且只有匹配的 Chat row 才会消费它。调用方不会查询 Chat DOM。
...
详情壳层声明会话作用域的 `conversation.details.pinned` 列表，供必须与 Chat 同屏的持久面板使用。没有占用者时，工具详情保留原有的全高度树和关闭行为。有占用者时，常驻内容会填满该列，直到工具 selection 打开一个最高占 45% 的底部抽屉；关闭该抽屉只清除工具 selection。占用状态通过 slot 账本 observable 跟随晚注册与 dispose，详见[常驻详情决策](../../../.agents/notes/implemented/architecture/2026-08-14-pinned-conversation-details-slot.md)。
