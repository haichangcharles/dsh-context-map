...
function makeHarness(
  ...
  const renderNodeSlot = ((key: string, owner: object, opts?: {
    ...
    switch (nodeOwner.node.kind) {
      case 'user':
      case 'steering':
        return <UserMessageNodeView {...nodeProps<'user' | 'steering'>()} renderSlot={() => null} />
      case 'context':
...
describe('ChatView', () => {
  ...
  it('loads and reveals the exact durable message requested by another view', async () => {
    const h = makeHarness({ nodes: [userInTurn(4, 'second prompt', 2)] })
    const complete = vi.fn()
    const view = render(<h.ChatView {...h.props} />)
    const row = view.container.querySelector('[data-chat-message-seq="4"]') as HTMLElement
    const scroll = vi.fn()
    row.scrollIntoView = scroll
    await act(async () => {
      view.rerender(<h.ChatView {...h.props} viewRequest={{ view: 'chat', focus: 'message:4' }} completeViewRequest={complete} />)
    })
    expect(h.loadThrough).toHaveBeenCalledWith(4)
