...
export function ChatView({
  ...
  const [messageReady, setMessageReady] = useState<string | null>(null)
  useEffect(() => {
    if (viewRequest?.view !== 'chat' || !/^message:\d+$/.test(viewRequest.focus)) return
    let cancelled = false
    const focus = viewRequest.focus
    void loadThrough(Number(focus.slice(8)) as SessionSeq).then(() => {
      if (!cancelled) setMessageReady(focus)
    }, () => {
      if (!cancelled) completeViewRequest()
    })
    return () => { cancelled = true }
  }, [viewRequest, loadThrough, completeViewRequest])
  ...
  useLayoutEffect(() => {
    if (messageReady === null || viewRequest?.focus !== messageReady) return
    const row = listRef.current?.querySelector<HTMLElement>(`[data-chat-message-seq="${Number(messageReady.slice(8))}"]`)
    if (row === null || row === undefined) return
    const turn = Number(row.dataset.chatTurn)
    const process = nodeStore.get(row.dataset.chatAnchorKey ?? '') as import('../contract/chat-nodes.ts').ChatNode | undefined
    if (process?.kind === 'assistant-step') actions.setTurnProcessOpen(turn, process.data.step, true)
    atBottomRef.current = false
    setAtBottom(false)
    row.scrollIntoView({ block: 'center' })
    row.animate?.([{ outline: '2px solid currentColor' }, { outline: '2px solid transparent' }], { duration: 1800 })
    setMessageReady(null)
    completeViewRequest()
  }, [messageReady, viewRequest, order, nodeStore, actions, completeViewRequest])
