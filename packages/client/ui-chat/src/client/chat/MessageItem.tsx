...
/** Steering reuses the message bubble without declaring user action slots. */
export const SteeringMessageNodeView = memo(function SteeringMessageNodeView(props: ChatNodeViewProps<'steering'>) {
  return <UserMessageNodeView {...props} />
})

/** User and admitted-steering keyed Chat renderer. */
export const UserMessageNodeView = memo(function UserMessageNodeView({
  node, renderMessageImages, renderSlot, t,
}: ChatNodeViewProps<'user' | 'steering'> & Partial<Pick<PropsRenderSlots<'conversation.chat.user-actions'>, 'renderSlot'>>) {
    ...
    <UserStyleBubble
      ...
      actions={text => (
        <><MessageIconActions
          ...
          className={css.actions}
          t={t}
        />{node.kind === 'user' && renderSlot?.('conversation.chat.user-actions', { seq: data.seq })}</>
      )}
    />
  )
})
