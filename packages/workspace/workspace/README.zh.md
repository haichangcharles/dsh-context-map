...
- `ctx.workspaceRegistry.archiveSession(id)`／`unarchiveSession(id)`／`archivedSessionIds`：覆盖在 workspace 记账之上的注册表级全局归档集合。被归档的会话从各分组视图中消失，但保留会话日志与 `sessionIds` 席位，因此恢复后按当前记账与顺序重新出现。两种变更都与其他注册表写入串行且具幂等性。恢复只移除同一个稳定 Session id，绝不回放归档时的 Workspace 快照，因此归档期间已提交的 Workspace 重命名、重排、删除或重新记账拥有优先权。若已归档 id 对应的实时／持久 Session 已消失，恢复会在不改变集合的情况下拒绝。在该字段出现之前写入的状态解析为空集合。
