...
- `ctx.workspaceRegistry.archiveSession(id)` / `unarchiveSession(id)` / `archivedSessionIds` — the registry-global archive set, layered over workspace accounting: an archived session disappears from grouping surfaces but keeps its session log and its `sessionIds` slot, so restore returns it to the current account and order. Both mutations are serialized with the other registry writes and are idempotent. Restore removes only the same stable Session id; it never replays an archived Workspace snapshot, so Workspace rename, reorder, deletion, or re-accounting committed while the Session was archived wins. Restoring an archived id whose live/persisted Session has disappeared rejects without changing the set. State written before the field existed parses with an empty set.
